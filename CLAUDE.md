...
> 기준: 2026-08-14. 입력창 폰트 통일 + 휴대폰번호/내선번호 자동 하이픈 포맷까지 커밋·push 예정, 브라우저에서 동작 검증 완료(직원 등록 폼 날짜 폰트, 휴대폰번호 자동 포맷, 내선번호 짧은 번호 하이픈 미적용 모두 확인). 이 커밋 이후 CLAUDE.md "배포 자동화 규칙"에 따라 테스트 서버는 자동 배포하고, 운영 배포는 사용자에게 물어볼 것. **미완료 항목: `KAKAO_REST_API_KEY` 테스트 환경 시크릿을 사용자가 아직 등록하지 않음** — `wrangler secret put KAKAO_REST_API_KEY --env test`를 backend 디렉터리에서 직접 실행해야 근무지 등록(지오코딩) 기능이 테스트 환경에서도 동작함. 그 외 기능은 이미 정상 동작.
