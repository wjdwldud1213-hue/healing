...
> 기준: 2026-08-14. 무한 리다이렉트 버그 수정(커밋 `9230e37`)까지 커밋·push·운영/테스트 배포 완료, 운영 배포본에서 실제 재현 후 수정 확인함. **워킹트리 깨끗함 — 미커밋 변경 없음.**
>
> **사용자가 아직 실제 카카오 계정으로 로그인→연동→비밀번호 재설정까지 전체 흐름을 직접 완주해보지 않음** — 코드 레벨(에러 처리, authorize URL 생성, redirect_uri 유효성, 라우팅, 메뉴 제거, 이번에 고친 무한 리다이렉트 등)은 검증됐지만 실제 완주는 사용자 본인 카카오 계정이 필요해서 다음 세션 전에 사용자가 직접 테스트할 예정. 문제가 있으면 그 내용을 갖고 다시 요청할 것.
>
> **미완료 항목(기존, 이번 건과 별개)**: `KAKAO_REST_API_KEY` 테스트 환경 시크릿이 아직 미등록 — `wrangler secret put KAKAO_REST_API_KEY --env test`. 이게 없으면 테스트 환경에서는 근무지 지오코딩뿐 아니라 카카오 로그인(연동/재설정)도 `client_id=undefined`로 실패한다(운영은 이미 등록되어 있어 정상 동작 확인함).
