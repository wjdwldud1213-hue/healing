...
> 기준: 2026-08-15. 모달 드래그-닫힘 버그 수정(커밋 `c15bb03`)까지 커밋·push·운영/테스트 배포 완료. **워킹트리 깨끗함 — 미커밋 변경 없음.** 카카오 계정 연동 기반 셀프 비밀번호 재설정 기능이 운영에서 실사용자 계정(A0003)으로 전 과정(연동/로그아웃 후 재인증/비밀번호 변경) 실제 완주 검증까지 끝남 — 이 기능은 이제 완전히 검증된 상태.
>
